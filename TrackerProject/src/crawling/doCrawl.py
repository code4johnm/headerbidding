...
project_root = script_dir.parents[3]  # HB-update root from .../src/crawling/
...
class TrainingCrawl:
    ...
    def initConfigParams(self, description=""):
        ...
        with open(browserParamsPath) as f:
            prefs = json.load(f)
            browser_params = [copy.deepcopy(prefs) for i in range(0, self.NUM_BROWSERS)]
            ...
            try:
                ...
                configParams["manager_params"]["data_directory"] = (
                    "results/data/{}".format(path)
                )
                configParams["manager_params"]["log_path"] = (
                    "results/logs/{}/openwpm-training-{}.log".format(path, description)
                )
                configParams["manager_params"]["database_name"] = (
                    "crawl-data-training-{}.sqlite".format(description)
                )
                configParams["browser_params"][0]["profile_archive_dir"] = (
                    "profiles/training/{}".format(path)
                )
    ...
    def crawlSites(self, sitesToVisit, managers):
        ...
        for index in range(len(managers)):
            ...
            try:
                ...
                # dump_profile_cookies removed; cookies via http_instrument/cookie_instrument now
                ...
                msg = "Exception {} occurred, cleaning up and going on to next site".format(
                    e
                )
                self.hblogger.log(msg, level="ERROR")
