...
class MLTesting:
    ...
    def archive(self, folder, dst, crawl_type):
        os.system(
            "tar -C {} -cvf /mnt/hgfs/archive_save/ml_testing/ready_for_analysis/{}_{}.tar training testing".format(
                folder, dst, crawl_type
            )
        )
    ...
    def monitor_thread(self):
        ...
        while 1:
            ...
            for profile in self.tp:
                ...
                ninety_minutes_old = (
                    int(self.training_profiles[profile]["timestamp"]) + 5400
                )
                if int(time.time()) > ninety_minutes_old:
                    ...
                    if retVal == "fail":
                        msg = (
                            "MONITOR THREAD - Training profile folder empty {}".format(
                                profile
                            )
                        )
    ...
    def run_testing(self, profile):
        ...
        msg = "Copying training profile {} to testing profile folder {} ".format(
            training_profile + "/*", testing_profile_folder
        )
        ...
        finalized_path = os.path.join(
            "/mnt/hgfs/archive_save/ml_testing", "ready_for_analysis", file_name
        )
        ...
        try:
            ...
            msg = "timestamp {} crawl_type {}, site {} category {}".format(
                timestamp, crawl_type, site, category
            )

            self.hblogger.log(msg)
            os.system(
                "python testingCrawl.py --timestamp {} --crawl_type {} --site {} --category {}".format(
                    timestamp, crawl_type, site, category
                )
            )
            # Brosermobproxy process sticks around, I need to figure out how to gracefully close it
            ...
            msg = (
                "moving testing results {} to finalized results folder {}/bids ".format(
                    testing_bids, testing_results
                )
            )
            ...
            os.system(
                "mv {} {} ".format(testing_bids, os.path.join(testing_results, "bids"))
            )
            os.system(
                "mv {} {}".format(testing_owpm, os.path.join(testing_results, "owpm"))
            )
