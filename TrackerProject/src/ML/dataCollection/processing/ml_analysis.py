...
class ml_analysis:
    ...
    def get_bids(self, id, timestamp, category, volume, intent, entity, path):
        ...
        for bids_file in bid_files:
            ...
            if bids:
                ...
                with open(
                    "bidcheck/{}_{}".format(bids_file, self.fileConter), "w+"
                ) as f:
                    json.dump(bids, f)
                    self.fileConter += 1
            for cat in bids:
                            ...
                            try:
                                ...
                                if bidder in self.bidders_per_category:
                                    if category in self.bidders_per_category[bidder]:
                                        if (
                                            "count"
                                            in self.bidders_per_category[bidder][
                                                category
                                            ]
                                        ):
                                            self.bidders_per_category[bidder][category][
                                                "count"
                                            ] += 1
                                        else:
                                            self.bidders_per_category[bidder][category][
                                    ...
                                    self.bidders_per_category[bidder] = {
                                        category: {"count": 1, "cpm": cpm}
                                    }
    ...
    def get_trackers(self, path, category, ml_type="training"):
        ...
        os.system(
            "python measure_tools/get_tp_and_owners.py --sqlite_file {} --output {}".format(
        ...
        os.system(
            "python measure_tools/parse_1.py --output {} --tp_file {}".format(
                self.output_path, tp_path
            )
        )
        os.system(
            "python measure_tools/parse_2.py --output {}".format(self.output_path)
        )
        os.system(
            "python measure_tools/parse_3.py --output {} --ml_type {}".format(
                self.output_path, ml_type
            )
        )
        trackers_seen = []
        tracker_count = {}
        ...
        track_count_path = ""
        if ml_type == "training":
            tracker_count_path = os.path.join(
                self.output_path, "training_trackers_count.json"
            )
        else:
            tracker_count_path = os.path.join(
                self.output_path, "testing_trackers_count.json"
            )
            ...
            if tracker in self.tracker_count:
                ...
                if ml_type in self.tracker_count_per_category:
                    if tracker in self.tracker_count_per_category[ml_type]:
                        if (
                            category
                            in self.tracker_count_per_category[ml_type][tracker]
                        ):
                            self.tracker_count_per_category[ml_type][tracker][
                                category
                            ] += count
                        else:
                            self.tracker_count_per_category[ml_type][tracker][
                    ...
                    self.tracker_count_per_category[ml_type] = {
                        tracker: {category: count}
                    }
        ...
        for tracker in tracker_count:
            if total != 0:
                if tracker in trackers_seen:
                    tracker_percentage[tracker] = float(tracker_count[tracker]) / float(
                        total
                    )
    ...
    def get_features(self):
        ...
        for t in analysis_tars:
            try:
                ...
                if "tars_processed" in self.save_data:
                    if self.save_data["tars_processed"]:
                        if t in self.save_data["tars_processed"]:
                            print(
                                "[ML ANALYSIS] {} - SKIPPING TAR {} - {}/{}".format(
                                    time.asctime(), t, tars_processed, total_tars
                                )
                            )
                            tars_processed += 1
                            continue
                ...
                print(
                    "\n[ML ANALYSIS] {:<20} - PROCESSING {:<20} - {}/{}".format(
                        time.asctime(), t, tars_processed, total_tars
                    )
                )
                os.system("rm -rf {}/*".format(self.output_path))
                ...
                if reset:
                    ...
                    print(
                        "[ML ANALYSIS] {} - EMPTY BIDS - SKIPPING {}".format(
                            time.asctime(), t
                        )
                    )
                ...
                print(
                    "[ML ANALYSIS] {:<20} - PROCESSING TRAINING TRACKERS".format(
                        time.asctime()
                    )
                )
                trackers_in_training = self.get_trackers(
                    tmp_path, category, ml_type="training"
                )
                all_rows["training"].append(trackers_in_training)

                print(
                    "[ML ANALYSIS] {:<20} - PROCESSING TESTING TRACKERS".format(
                        time.asctime()
                    )
                )
                trackers_in_testing = self.get_trackers(
                    tmp_path, category, ml_type="testing"
                )
            ...
            for tracker in sorted(self.tracker_count_per_category[ml_type]):
                ...
                for c in categories:
                    if c in self.tracker_count_per_category[ml_type][tracker]:
                        category_output += "{:<9}".format(
                            self.tracker_count_per_category[ml_type][tracker][c]
                        )
                        if c in totalTrackers[ml_type]:
                            totalTrackers[ml_type][
                                c
                            ] += self.tracker_count_per_category[ml_type][tracker][c]

                        else:
                            totalTrackers[ml_type][c] = self.tracker_count_per_category[
                                ml_type
                            ][tracker][c]
                    else:
                    ...
                    if c in self.tracker_count_per_category[ml_type][tracker]:
                        if c in totalTrackers[ml_type]:
                            if c in totalTrackers_PCT:
                                totalTrackers_PCT[ml_type][c] += float(
                                    self.tracker_count_per_category[ml_type][tracker][c]
                                ) / float(totalTrackers[ml_type][c])
                            else:
                                totalTrackers_PCT[ml_type][c] = float(
                                    self.tracker_count_per_category[ml_type][tracker][c]
                                ) / float(totalTrackers[ml_type][c])
        ...
        for bidder in self.bidders_per_category:
            ...
            for c in categories:
                if c in self.bidders_per_category[bidder]:
                    category_output += "{:<9}".format(
                        self.bidders_per_category[bidder][c]["count"]
                    )
                    if bidder in totalCpmPerBidder:
                        if "count" in totalCpmPerBidder:
                            totalCpmPerBidder[bidder][
                                "count"
                            ] += self.bidders_per_category[bidder][c]["count"]
                        else:
                            totalCpmPerBidder[bidder]["count"] = (
                                self.bidders_per_category[bidder][c]["count"]
                            )
                    else:
                        totalCpmPerBidder[bidder] = {
                            "count": self.bidders_per_category[bidder][c]["count"]
                        }
                else:
                    category_output += "{:<9}".format(0)
            ...
            for c in categories:
                if c in self.bidders_per_category[bidder]:
                    category_output += "{:<9.3f}".format(
                        self.bidders_per_category[bidder][c]["cpm"]
                    )
                    if bidder in totalCpmPerBidder:
                        if "cpm" in totalCpmPerBidder:
                            ...
                            totalCpmPerBidder[bidder]["cpm"] = (
                                self.bidders_per_category[bidder][c]["cpm"]
                            )
                    else:
                        totalCpmPerBidder[bidder].update(
                            {"cpm": self.bidders_per_category[bidder][c]["cpm"]}
                        )
                else:
        ...
        for bidder in totalCpmPerBidder:
            ...
            category_output += "${:<9.3f}\n\n".format(
                float(totalCpmPerBidder[bidder]["cpm"])
                / float(totalCpmPerBidder[bidder]["count"])
            )
    ...
    def generate_arff(
        self, cols, rows, ids, bidders, mediaTypes, blockingEntities, sizes
    ):
        attributes = []
        data = []
