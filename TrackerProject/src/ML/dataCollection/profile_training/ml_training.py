...
def archive_training(timestamp, category, intent, training_profile_logs):
    ...
    if os.path.exists("{}".format(crawl_dir)):
        ...
        os.system(
            "tar -C {} -cvf {}_TRAINING.tar profile results manifest_{}.json".format(
                crawl_dir, crawl_dir, timestamp
            )
        )
...
while trainingRounds < 100000:
    ...
    for blocking_profile in blocking_profiles:
        if skip_block_profile > 0:
            skip_block_profile -= 1
            msg = "BLOCKING PROFILE {}, skip_block_profile {}".format(
                blocking_profile, skip_block_profile
            )
            ...
            if blocking_profile == "all":
                ...
                fn_all = os.path.join(
                    "ent_abp",
                    top_twenty_all_filters[tracker_to_block].split("-filter")[0]
                    + ".txt",
                )
            ...
            if filter_name != "":
                ...
                with open("./firefox_extensions/ublock_origin/storage.js") as f:
                    ...
                    content_url = ublock_origin_rules["availableFilterLists"][
                        filter_name
                    ]["contentURL"]
                    ublock_origin_rules["selectedFilterLists"] = ["ublock-filters"]
                    ublock_origin_rules["availableFilterLists"]["ublock-filters"][
                        "contentURL"
                    ] = content_url
                ...
                with open(
                    "./src/ublock_origin-1.14.10/assets/ublock/filters.txt", "w"
                ) as f:
                    for r in rules:
                        f.write(r)
                ...
                os.system(
                    "cd {}; zip -r -FS ../{} *;cd ../".format(ublock_folder, local_xpi)
                )
        ...
        for trainingTypes in intentTrainingTypes:
            if skip_intent > 0:
                ...
                msg = "TRAINING ROUNDS {} BLOCK_PROFILE {},  TRAINING INTENT TYPE {}".format(
                    trainingRounds, blocking_profile, trainingTypes
                )

                hblogger.info(msg)
                training_profile = {
                    "training_id": "{}_{}".format(trainingRounds, blocking_profile),
                ...
                try:
                    ...
                    #                                                                                                 categoryType))
                    os.system(
                        "python trainingCrawl.py --intent {} --timestamp {} --volume {} --category {} --blocking {} --tracker_ent {}".format(
                            trainingTypes,
                            timestamp,
                            numberOfSitesToVisit,
                            categoryType,
                    ...
                    archive_training(
                        timestamp, categoryType, trainingTypes, training_profile
                    )
                    # update_block_lists()
