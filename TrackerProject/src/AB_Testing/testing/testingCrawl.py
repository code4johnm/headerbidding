...
project_root = script_dir.parents[
    4
]  # HB-update root from .../AB_Testing/testing/training/
...
msg = "Starting Session: intent {} timestamp {}, category {} pbjs_site {}".format(
    crawl_type, timestamp, category, pbjs_site
)
...
with open("./config/testing/allow_browser_params.json") as f:
    prefs = json.load(f)
    allow_browser_params = [copy.deepcopy(prefs) for i in range(0, NUM_BROWSERS)]
...
with open("./config/testing/block_browser_params.json") as f:
    prefs = json.load(f)
    block_browser_params = [copy.deepcopy(prefs) for i in range(0, NUM_BROWSERS)]
...
for iab in iab_params:
    try:
        if iab != "BLOCK":
            iab_params[iab].update(
                {"manager_params": copy.deepcopy(allow_manager_params)}
            )
            iab_params[iab].update(
                {"browser_params": copy.deepcopy(allow_browser_params)}
            )
            data_dir = "./results/owpm/testing/{}/{}/".format(timestamp, iab)
            iab_params[iab]["manager_params"]["data_directory"] = data_dir
            iab_params[iab]["manager_params"]["log_path"] = (
                data_dir + "openwpm-testing-{}.log".format(iab)
            )
            iab_params[iab]["browser_params"][0]["seed_tar"] = "{}".format(profile_path)
            iab_params[iab]["browser_params"][0]["profile_archive_dir"] = (
                "./profiles/testing/{}/{}/".format(timestamp, iab)
            )
        else:
            iab_params[iab].update(
                {"manager_params": copy.deepcopy(block_manager_params)}
            )
            iab_params[iab].update(
                {"browser_params": copy.deepcopy(block_browser_params)}
            )
            data_dir = "./results/owpm/testing/{}/{}/".format(timestamp, iab)
            iab_params[iab]["manager_params"]["data_directory"] = data_dir
            iab_params[iab]["manager_params"]["log_path"] = (
    ...
    try:
        ...
        for visit in [1]:
            ...
            for iab in managers:
                ...
                site = pbjs_sites[rank]
                msg = "[TESTING] {}, VISITING SITE: {} RANK: {}, IAB: {}".format(
                    site, rank, iab
                )
