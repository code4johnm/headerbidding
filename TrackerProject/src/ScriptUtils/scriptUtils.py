...
class ScriptUtils:
    ...
    def getCpm(self, iab, visit, site, crawl_type, **kwargs):
        ...
        try:
            ...
            print(
                "[SCRIPT UTILS] {} Exception occurred trying to open file {}".format(
                    time.asctime(), e
                )
            )
            retry = 0
            ...
            while retry < 20:
                try:
                    ...
                    print(
                        "[SCRIPT UTILS] {} Exception occurred trying to open file {}, retrying: {}".format(
                            time.asctime(), e, retry
                        )
                    )
                    retry += 1
            ...
            if retry >= 20:
                print(
                    "[SCRIPT UTILS] {} Exception occurred trying to open file {}, quitting...: {}".format(
                        time.asctime(), e, retry
                    )
                )
        ...
        if crawl_type == "NO_INTENT":
            file = os.path.join(
                "./results/bids_no_intent/", site + "_{}.json".format(iab)
            )
        else:
        ...
        with open("testingDone.json", "w") as f:
            print(
                "[SCRIPT UTILS] {} writing {} to {}".format(
                    iab, done, "testingDone.json"
                )
            )
