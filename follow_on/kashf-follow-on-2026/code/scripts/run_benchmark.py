...
from kashf.metrics import edge_recovery_report, fdr_aware_precision_recall
...
def main():
    ...
    print("=" * 70)
    print(
        f"Difficulty: {args.difficulty} | Trackers: {args.n_trackers} | Advertisers: {args.n_advertisers}"
    )
    ...
    print(
        f"      {sum(len(p) for p in G.influence_paths.values())} total influence paths (incl. chains)"
    )
    print()
    ...
    true_direct = {
        (src, dst)
        for src, dst, _, _ in G.direct_edges
        if src.startswith("T") and dst.startswith("A")
    }
    true_any = set()
    ...
    method_edges = {
        ...
        f"causalkashf_fdr{int(args.target_fdr*100)}": set(
            comparison["causalkashf_calibrated"]["edges"]
        ),
    ...
    causal_recall = [r for r in rows if "causalkashf" in r["method"]][0][
        "recall_direct"
    ]
    ...
    print(
        f"  CausalKashf (target {args.target_fdr:.0%}) achieved empirical FDR = {causal_fdr:.1%}"
    )
