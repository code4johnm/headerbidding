...
G, df, harness = generate_and_run(
    difficulty="easy", n_trackers=18, n_advertisers=5, n_personas=800, seed=7
)
...
print(
    f"Original Kashf (top-3):  precision={k_m['precision']:.3f}  recall={k_m['recall_direct']:.3f}  empirical_FDR={k_m['fdr']:.3f}"
)
print(
    f"CausalKashf (target 10% FDR): precision={c_m['precision']:.3f}  recall={c_m['recall_direct']:.3f}  empirical_FDR={c_m['fdr']:.3f}"
)
print(
